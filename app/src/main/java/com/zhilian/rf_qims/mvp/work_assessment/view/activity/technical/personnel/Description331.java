 ...
 * 安装质量（3.3.1）
 */
public class Description331 extends BaseModuleActivity {
	@BindView(R.id.textView4)// 标准
		TextView mTextView4;
	@BindView(R.id.tv_value3_3_1)// 分值
		TextView mTvValue331;
	@BindView(R.id.tv_score3_3_1)// 得分
		TextView mTvScore331;
	@BindView(R.id.tv_rule3_3_1)// 扣分规则
		TextView mTvRule331;
	@BindView(R.id.cb1_3_3_1)// 缺技术交底资料的扣一半分
		CheckBox mCb1331;
	@BindView(R.id.cb2_3_3_1)// 考评现场未提供施工方案或方案未涉及预埋件的不得分
		CheckBox mCb2331;
	...
	protected int getLayoutRes() {
		return R.layout.dialog_description331;
	}
	...
	protected void initData() {
		...
		mTvValue331.setText(point);// 分值
		mTvScore331.setText(point);// 得分
		mTvRule331.setText(config.get(0).getMEMO());// 扣分说明

		if (list.size() > 0) {
			id = list.get(0).getID();
			status = list.get(0).getSTATUS();
			mTvScore331.setText(list.get(0).getSCORE()); //得分
			...
			if (pChoose[0].equals("1")) {// 缺技术交底资料的扣一半分
				mCb1331.setChecked(true);
			}
			if (pChoose[1].equals("1")) {// 考评现场未提供施工方案或方案未涉及预埋件的不得分
				mCb2331.setChecked(true);
			}
		}
	}
	...
	protected void listener() {
		...
		// 缺技术交底资料的扣一半分
		mCb1331.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					if (mCb2331.isChecked()){
						mTvScore331.setText(ScoreUtil.SCORE_NOT);
					}else {
						mTvScore331.setText(ScoreUtil.scoreHalf(point));
					}
				}else {
					if (mCb2331.isChecked()){
						mTvScore331.setText(ScoreUtil.SCORE_NOT);
					}else {
						mTvScore331.setText(ScoreUtil.scoreAll(point));
					}
				}
				saveData(status);
			}
		...
		// 考评现场未提供施工方案或方案未涉及预埋件的不得分
		mCb2331.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					mTvScore331.setText(ScoreUtil.SCORE_NOT);
				}else {
					if (mCb1331.isChecked()){
						mTvScore331.setText(ScoreUtil.scoreHalf(point));
					}else {
						mTvScore331.setText(ScoreUtil.scoreAll(point));
					}
	...
	private void saveData(String status) {
		...
		int condition1;
		if (mCb1331.isChecked()) {// 自动焊工艺每批均有抽检记录
			condition1 = 1;
		} else {
			condition1 = 0;
		}
		...
		int condition2;
		if (mCb2331.isChecked()) {// 门扇、门框成品检查记录不齐全不得分
			condition2 = 1;
		} else {
			condition2 = 0;
		}
		...
		String point = String.valueOf(mTvScore331.getText());
