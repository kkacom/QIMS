 ...
 * 产品质量（3.2.5）
 */
public class Description325 extends BaseModuleActivity {
	@BindView(R.id.textView4)// 标准
		TextView mTextView4;
	@BindView(R.id.tv_value3_2_5)// 分值
		TextView mTvValue325;
	@BindView(R.id.tv_score3_2_5)// 得分
		TextView mTvScore325;
	@BindView(R.id.tv_rule3_2_5)// 扣分规则
		TextView mTvRule325;
	@BindView(R.id.cb1_3_2_5)// 检查结果未覆盖每批次扣一半分
		CheckBox mCb1325;
	@BindView(R.id.cb2_3_2_5)// 未见检查结果不得分
		CheckBox mCb2325;
	...
	protected int getLayoutRes() {
		return R.layout.dialog_description20;
	}
	...
	protected void initData() {
		...
		mTvValue325.setText(point);// 分值
		mTvScore325.setText(point);// 分值
		mTvRule325.setText(config.get(0).getMEMO());// 扣分说明

		if (list.size() > 0) {
			id = list.get(0).getID();
			status = list.get(0).getSTATUS();
			mTvScore325.setText(list.get(0).getSCORE()); //得分
			...
			if (pChoose[0].equals("1")) {// 检查结果未覆盖每批次扣一半分
				mCb1325.setChecked(true);
			}
			if (pChoose[1].equals("1")) {// 未见检查结果不得分
				mCb2325.setChecked(true);
			}
		}
	}
	...
	protected void listener() {
		...
		// 检查结果未覆盖每批次扣一半分
		mCb1325.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					if (mCb2325.isChecked()){
						mTvScore325.setText(ScoreUtil.SCORE_NOT);
					}else {
						mTvScore325.setText(ScoreUtil.scoreHalf(point));
					}
				}else {
					mTvScore325.setText(ScoreUtil.scoreAll(point));
				}
				saveData(status);
			}
		});
		...
		// 未见检查结果不得分
		mCb2325.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					mTvScore325.setText(ScoreUtil.SCORE_NOT);
				}else {
					if (mCb1325.isChecked()){
						mTvScore325.setText(ScoreUtil.scoreHalf(point));
					}else {
						mTvScore325.setText(ScoreUtil.scoreAll(point));
					}
	...
	private void saveData(String status) {
		...
		int condition1;
		if (mCb1325.isChecked()) {// 自动焊工艺每批均有抽检记录
			condition1 = 1;
		} else {
			condition1 = 0;
		}
		...
		int condition2;
		if (mCb2325.isChecked()) {// 门扇、门框成品检查记录不齐全不得分
			condition2 = 1;
		} else {
			condition2 = 0;
		}
		...
		String point = String.valueOf(mTvScore325.getText());
