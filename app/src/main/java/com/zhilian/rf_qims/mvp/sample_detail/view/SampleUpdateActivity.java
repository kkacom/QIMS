...
public class SampleUpdateActivity extends AppCompatActivity {
	...
	private Map<String, String> testPerson;
	...
	private void initView() {
		...
		testPerson = new HashMap<>();
		for (int i = 0; i < checkUsers.size(); i++) {
			items[i] = checkUsers.get(i).getNAME();
			testPerson.put(String.valueOf(checkUsers.get(i).getID()), checkUsers.get(i).getNAME());
		}
...
//                updateSampleDesign(tvSamplemodel.getText().toString().trim());
		...
		mTvMainuser.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				AlertDialog.Builder builder = new AlertDialog.Builder(SampleUpdateActivity.this);
...
//                        Toast.makeText(SampleDetailActivity2.this,"您选择了:"+items[which],Toast.LENGTH_SHORT).show();
						mTvMainuser.setText(items[which]);
...
//                        Toast.makeText(SampleDetailActivity2.this,"您点击了"+which,Toast.LENGTH_SHORT).show();
		...
		mTvFuuser.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				fuuserid = "";
				...
				builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						mTvFuuser.setText(text.substring(0, text.length() - 1));
	...
	private void setViewData() {
		...
		if (sample.getTester() != null) {
			mainuserid = Integer.valueOf(sample.getTester());
			mTvMainuser.setText(testPerson.get(sample.getTester()));
		}
		if (sample.getTester1() != null) {
			if (!sample.getTester1().equals("")) {
				String[] persons = sample.getTester1().split(",");
				String str = "";
				for (int i = 0; i < persons.length; i++) {
					if (i == 0 && i == persons.length - 1) { //只有一个辅检人
						str = testPerson.get(persons[i]);
					} else if (i == 0) {                        //第一个辅检人，且不止一个
						str = testPerson.get(persons[i]) + ",";
					} else if (i == persons.length - 1) {        //最后一个辅检人
						str = str + testPerson.get(persons[i]);
					} else {                                    //第2 ~ persons[] -1 个人
						str = str + testPerson.get(persons[i]) + ",";
					}
				}
				fuuserid = sample.getTester1();
				mTvFuuser.setText(str);
			}
		}
...
//        popupWindow.showAtLocation(tvSampleno, Gravity.NO_GRAVITY, location[0], location[1] + tvSampleno.getHeight());
	...
	private void save() {
		...
		if (mTvSitenumber.getText().toString().trim().equals("")) {
			Toast.makeText(SampleUpdateActivity.this, "现场编号不能为空", Toast.LENGTH_SHORT).show();
			return;
		}
