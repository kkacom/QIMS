 ...
 */
public class FragmentDesignDemo extends BaseListFragment {
    ...
    public void onEvent(EventUtil event) {
        System.out.println("---->>yyyyyyyy"+event.getMsg());
        try {
            Thread.sleep(1000);
    ...
    Runnable eChanged = new Runnable() {
        ...
        public void run() {
            creditList.clear();
            creditList.addAll(EnterpriseCreditDao.query(4));
     ...
     */
    private void data(){
        creditList = EnterpriseCreditDao.query(4);
        mAdapter = new DemoAdapter(getActivity(), creditList);
        mListView.setAdapter(mAdapter);
        if(creditList.size() > 0){
            mListBg.setVisibility(View.GONE);
