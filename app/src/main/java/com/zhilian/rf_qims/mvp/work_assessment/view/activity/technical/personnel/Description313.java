 ...
 * 经营管理（3.1.2）
 */
public class Description313 extends BaseModuleActivity {

	@BindView(R.id.textView4)// 标题
		TextView mTextView4;
	@BindView(R.id.tv_value3_1_3)// 分值
		TextView mTvValue313;
	@BindView(R.id.tv_rule3_1_3)// 得分
		TextView mTvRule313;
	@BindView(R.id.tv_score3_1_3)// 扣分规则
		TextView mTvScore313;
	@BindView(R.id.cb1_3_1_3)// 现场使用没有确认的图纸扣一半分
		CheckBox mCb1313;
	@BindView(R.id.cb2_3_1_3)// 加大设计未经技术确认扣一半分
		CheckBox mCb2313;
	@BindView(R.id.cb3_3_1_3)// 现场使用没有依据的图纸不得分
		CheckBox mCb3313;
	...
	protected int getLayoutRes() {
		return R.layout.dialog_description313;
	}
	...
	protected void initData() {
		...
		mTvValue313.setText(point);// 分值
		mTvScore313.setText(point);// 分值
		mTvRule313.setText(config.get(0).getMEMO().equals("") ? "无计分规则" : config.get(0).getMEMO());// 扣分说明

		if (list.size() > 0) {
			id = list.get(0).getID();
			status = list.get(0).getSTATUS();
			mTvScore313.setText(list.get(0).getSCORE()); //得分
			...
			if (pChoose[0].equals("1")) {// 现场使用没有确认的图纸扣一半分
				mCb1313.setChecked(true);
			}
			if (pChoose[1].equals("1")) {// 加大设计未经技术确认扣一半分
				mCb2313.setChecked(true);
			}
			if (pChoose[2].equals("1")) {// 现场使用没有依据的图纸不得分
				mCb3313.setChecked(true);
			}
		}
	}
	...
	protected void listener() {
		...
		// 现场使用没有确认的图纸扣一半分
		mCb1313.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked){
					if (mCb2313.isChecked()){// 加大设计未经技术确认扣一半分
						mTvScore313.setText(ScoreUtil.SCORE_NOT);
					}else {
						if (mCb3313.isChecked()){// 现场使用没有依据的图纸不得分
							mTvScore313.setText(ScoreUtil.SCORE_NOT);
						}else {
							mTvScore313.setText(ScoreUtil.scoreHalf(point));
						}
					}
				}else {
					if (mCb2313.isChecked()){// 加大设计未经技术确认扣一半分
						...
						if (mCb3313.isChecked()){// 现场使用没有依据的图纸不得分
							...
							mTvScore313.setText(ScoreUtil.scoreAll(point));
						}
					}
				}
				saveData(status);
		...
		// 加大设计未经技术确认扣一半分
		mCb2313.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked){
					if (mCb1313.isChecked()){// 现场使用没有确认的图纸扣一半分
						mTvScore313.setText(ScoreUtil.SCORE_NOT);
					}else {
						if (mCb3313.isChecked()){// 现场使用没有确认的图纸扣一半分
							mTvScore313.setText(ScoreUtil.SCORE_NOT);
						}else {
							mTvScore313.setText(ScoreUtil.scoreHalf(point));
						}
		...
		// 现场使用没有依据的图纸不得分
		mCb3313.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked){
					mTvScore313.setText(ScoreUtil.SCORE_NOT);
	...
	private void saveData(String status) {
		...
		int condition1;
		if (mCb1313.isChecked()) {// 现场验查有不良经营行为不得分
			condition1 = 1;
		} else {
			condition1 = 0;
		}
		...
		int condition2;
		if (mCb2313.isChecked()) {// 近两年有公开处罚的不得分
			condition2 = 1;
		} else {
			condition2 = 0;
		}
		...
		int condition3;
		if (mCb3313.isChecked()) {// 合同显示有恶意竞争倾向不得分
			condition3 = 1;
		} else {
			condition3 = 0;
		}
		...
		String point = String.valueOf(mTvScore313.getText());// 得分
