 ...
 * 产品质量（3.2.4）
 */
public class Description324 extends BaseModuleActivity {
	@BindView(R.id.textView4)// 标准
		TextView mTextView4;
	@BindView(R.id.tv_value3_2_4)// 得分
		TextView mTvValue324;
	@BindView(R.id.tv_score3_2_4)// 分值
		TextView mTvScore324;
	@BindView(R.id.tv_rule3_2_4)// 扣分规则
		TextView mTvRule324;
	@BindView(R.id.cb1_3_2_4)// 混凝土浇筑前检验记录钢筋直径、条数及间距、留足代表性试块
		CheckBox mCb1324;
	@BindView(R.id.cb2_3_2_4)// 无记录、记录不详不得分
		CheckBox mCb2324;
	...
	protected int getLayoutRes() {
		return R.layout.dialog_description19;
	}
	...
	protected void initData() {
		...
		mTvValue324.setText(point);// 分值
		mTvRule324.setText(config.get(0).getMEMO());// 扣分说明

		if (list.size() > 0) {
			id = list.get(0).getID();
			status = list.get(0).getSTATUS();
			mTvScore324.setText(list.get(0).getSCORE()); //得分
			...
			if (pChoose[0].equals("1")) {// 混凝土浇筑前检验记录钢筋直径、条数及间距、留足代表性试块
				mCb1324.setChecked(true);
			}
			if (pChoose[1].equals("1")) {// 无记录、记录不详不得分
				mCb2324.setChecked(true);
			}
		}
	}
	...
	protected void listener() {
		...
		// 混凝土浇筑前检验记录钢筋直径、条数及间距、留足代表性试块
		mCb1324.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					if (mCb2324.isChecked()){
						mTvScore324.setText(ScoreUtil.SCORE_NOT);
					}else {
						mTvScore324.setText(ScoreUtil.scoreAll(point));
					}
				}else {
					if (mCb2324.isChecked()){
						mTvScore324.setText(ScoreUtil.SCORE_NOT);
		...
		// 无记录、记录不详不得分
		mCb2324.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					mTvScore324.setText(ScoreUtil.SCORE_NOT);
				}else {
					if (mCb1324.isChecked()){
						mTvScore324.setText(ScoreUtil.scoreAll(point));
					}else {
						mTvScore324.setText("");
					}
	...
	private void saveData(String status) {
		...
		int condition1;
		if (mCb1324.isChecked()) {// 自动焊工艺每批均有抽检记录
			condition1 = 1;
		} else {
			condition1 = 0;
		}
		...
		int condition2;
		if (mCb2324.isChecked()) {// 门扇、门框成品检查记录不齐全不得分
			condition2 = 1;
		} else {
			condition2 = 0;
		}
		...
		String point = String.valueOf(mTvScore324.getText());
