 ...
 * 产品质量（3.2.3）
 */
public class Description323 extends BaseModuleActivity {
	@BindView(R.id.textView4)// 标题
		TextView mTextView4;
	@BindView(R.id.tv_value3_2_3)// 分值
		TextView mTvValue323;
	@BindView(R.id.tv_score3_2_3)// 得分
		TextView mTvScore323;
	@BindView(R.id.tv_rule3_2_3)// 扣分规则
		TextView mTvRule323;
	@BindView(R.id.cb1_3_2_3)// 自动焊工艺每批均有抽检记录
		CheckBox mCb1323;
	@BindView(R.id.cb2_3_2_3)// 门扇、门框成品检查记录不齐全不得分
		CheckBox mCb2323;
	@BindView(R.id.cb3_3_2_3)// 隐蔽项目缺检查记录不得分
		CheckBox mCb3323;
	@BindView(R.id.cb4_3_2_3)// 隐蔽项目检查未覆盖批次扣一半分
		CheckBox mCb4323;
	...
	protected int getLayoutRes() {
		return R.layout.dialog_description323;
	}
	...
	protected void initData() {
		...
		mTvValue323.setText(point);// 分值
		mTvRule323.setText(config.get(0).getMEMO());// 扣分说明

		if (list.size() > 0) {
			id = list.get(0).getID();
			status = list.get(0).getSTATUS();
			mTvScore323.setText(list.get(0).getSCORE()); //得分
			...
			if (pChoose[0].equals("1")) {// 自动焊工艺每批均有抽检记录
				mCb1323.setChecked(true);
			}
			if (pChoose[1].equals("1")) {// 门扇、门框成品检查记录不齐全不得分
				mCb2323.setChecked(true);
			}
			if (pChoose[2].equals("1")) {// 隐蔽项目缺检查记录不得分
				mCb3323.setChecked(true);
			}
			if (pChoose[3].equals("1")) {// 隐蔽项目检查未覆盖批次扣一半分
				mCb4323.setChecked(true);
			}
		}
	...
	protected void listener() {
		...
		// 自动焊工艺每批均有抽检记录
		mCb1323.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					if (mCb2323.isChecked()){
						mTvScore323.setText(ScoreUtil.SCORE_NOT);
					}else {
						if (mCb3323.isChecked()){
							mTvScore323.setText(ScoreUtil.SCORE_NOT);
						}else {
							if (mCb4323.isChecked()){
								mTvScore323.setText(ScoreUtil.scoreHalf(point));
							}else {
								mTvScore323.setText(ScoreUtil.scoreAll(point));
							}
						}
					}
				}else {
		...
		// 门扇、门框成品检查记录不齐全不得分
		mCb2323.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					mTvScore323.setText(ScoreUtil.SCORE_NOT);
				}else {
					if (mCb1323.isChecked()){
						if (mCb3323.isChecked()){
							mTvScore323.setText(ScoreUtil.SCORE_NOT);
						}else {
							if (mCb4323.isChecked()){
		...
		// 隐蔽项目缺检查记录不得分
		mCb3323.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					mTvScore323.setText(ScoreUtil.SCORE_NOT);
		...
		// 隐蔽项目检查未覆盖批次扣一半分
		mCb4323.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					if (mCb1323.isChecked()){
	...
	private void saveData(String status) {
		...
		int condition1;
		if (mCb1323.isChecked()) {// 自动焊工艺每批均有抽检记录
			condition1 = 1;
		} else {
			condition1 = 0;
		}
		...
		int condition2;
		if (mCb2323.isChecked()) {// 门扇、门框成品检查记录不齐全不得分
			condition2 = 1;
		} else {
			condition2 = 0;
		}
		...
		int condition3;
		if (mCb3323.isChecked()) {// 隐蔽项目缺检查记录不得分
			condition3 = 1;
		} else {
			condition3 = 0;
		}
		...
		int condition4;
		if (mCb4323.isChecked()) {// 隐蔽项目检查未覆盖批次扣一半分
			condition4 = 1;
		} else {
			condition4 = 0;
		}
		...
		String point = String.valueOf(mTvScore323.getText());
