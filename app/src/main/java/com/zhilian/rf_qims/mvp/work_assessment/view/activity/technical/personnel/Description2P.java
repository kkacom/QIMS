...
/**
 * 技术&生产人员（2.2.4-2.3.3高级）
 */
public class Description2P extends BaseModuleActivity {
	int mPosition = 0;
	Long rid;
	Long id;
	...
	RadioButton mRb7Yes215;// 有证书、无证书
	@BindView(R.id.rb7_no2_1_5)
	RadioButton mRb7No215;
	@BindView(R.id.rb8_yes2_1_5)
	RadioButton mRb8Yes215;// 有培训证书、无培训证书
	@BindView(R.id.rb8_no2_1_5)
	RadioButton mRb8No215;
	...
	protected int getLayoutRes() {
		return R.layout.activity_description2p;
	}
	...
	protected void initData() {
		...
		point = getIntent().getStringExtra("configPoint");//个人基础分值
		...
		if (personnelJson.getCHOOSE() != null){
			...
			if (pChoose[5].equals("1")) {// 0 有证书、1 无证书
				mRb7Yes215.setChecked(true);
			} else {
				mRb7No215.setChecked(true);
			}

			if (pChoose[6].equals("1")) {// 0 有培训证书、1 无培训证书
				mRb8Yes215.setChecked(true);
			} else {
				mRb8No215.setChecked(true);
			}
		}
	}
	...
	protected void listener() {
						...
						if (mRb4Yes215.isChecked()) {// 有社保
							if (mRb5Yes215.isChecked()) {// 有劳动合同
								if (mRb6Yes215.isChecked()) {// 考核满意
									if (mRb7Yes215.isChecked()) {// 有证书
										if (mRb8Yes215.isChecked()) {// 有培训证书
		...
		// 有培训证书 无培训证书
		mRb8Yes215.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (mRb1Yes215.isChecked()) {// 检查
					if (isChecked) {// 1 有培训证书
						if (mRb4Yes215.isChecked()) {// 有社保
							if (mRb5Yes215.isChecked()) {// 有劳动合同
								if (mRb6Yes215.isChecked()) {// 考核满意
									if (mRb7Yes215.isChecked()) {// 有证书
	...
	private void saveData(String status) {
		...
		int condition6;
		if (mRb7Yes215.isChecked()) {// 0 有证书、1 无证书
			condition6 = 1;
		} else {
			condition6 = 0;
		}
		...
		int condition7;
		if (mRb8Yes215.isChecked()) {// 0 有培训证书、1 无培训证书
			condition7 = 1;
		} else {
			condition7 = 0;
		}
