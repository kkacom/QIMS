...
/**
 * Created by colin on 2018/2/5 14:02 .
 */
...
public class InstallUtil {
	...
	public static void install(Activity activity, String apkPath, boolean rootMode){
		if (rootMode){
			installRoot(activity,apkPath);
		}else {
			installNormal(activity,apkPath);
		}
	}
	...
	private static void installNormal(Context context,String apkPath) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
			...
			context.grantUriPermission(packageName,apkUri, Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
	...
	//普通安装
	private static void installNormal(Activity activity,String apkPath) {
		Intent intent = new Intent(Intent.ACTION_VIEW);
		//版本在7.0以上是不能直接通过uri访问的
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
			File file = (new File(apkPath));
			...
			Uri apkUri = FileProvider.getUriForFile(activity, packageName+".fileprovider", file);
			activity.grantUriPermission(packageName,apkUri, Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
